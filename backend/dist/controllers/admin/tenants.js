...
exports.adminTenantsRouter = void 0;
...
router.get('/', async (req, res) => {
    try {
        ...
        const tenants = await appwrite.databases.listDocuments(process.env.APPWRITE_DATABASE_ID || 'bigtechdb', 'tenants', [] // Sem filtros - admin vê tudo
        );
        const formattedTenants = tenants.documents.map((doc) => ({
            id: doc.$id,
            name: doc.name,
            status: doc.status,
            plugins: doc.plugins || [],
...
router.post('/', async (req, res) => {
    try {
        ...
        const existing = await appwrite.databases.listDocuments(process.env.APPWRITE_DATABASE_ID || 'bigtechdb', 'tenants', [node_appwrite_1.Query.equal('name', name)]);
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const existing = await appwrite.databases.getDocument(process.env.APPWRITE_DATABASE_ID || 'bigtechdb', 'tenants', id);
        if (!existing) {
            return res.status(404).json({ error: 'Tenant not found' });
        }
        // Se nome mudou, validar unicidade
        if (name && name !== existing.name) {
            const nameCheck = await appwrite.databases.listDocuments(process.env.APPWRITE_DATABASE_ID || 'bigtechdb', 'tenants', [node_appwrite_1.Query.equal('name', name)]);
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        // Deletar tenant
        await appwrite.databases.deleteDocument(process.env.APPWRITE_DATABASE_ID || 'bigtechdb', 'tenants', id);
        // Auditar deleção
        await audit_1.auditLogger.log({
            tenantId: id,
            action: 'DELETE',
...
exports.adminTenantsRouter = router;

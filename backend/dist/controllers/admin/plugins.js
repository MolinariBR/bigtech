...
router.get('/', async (req, res) => {
    try {
        ...
        const plugins = availablePlugins.map(available => {
            const globalStatusStore = global.pluginStatusStore || {};
...
router.post('/', async (req, res) => {
    try {
        ...
        const existing = await appwrite.databases.listDocuments(process.env.APPWRITE_DATABASE_ID || 'bigtechdb', 'plugins', [node_appwrite_1.Query.equal('tenantId', tenantId), node_appwrite_1.Query.equal('pluginId', name)]);
...
router.post('/:pluginId/toggle', async (req, res) => {
    try {
        ...
        const globalStatusStore = global.pluginStatusStore || pluginStatusStore;
...
router.put('/:pluginId/config', async (req, res) => {
    try {
        ...
        const globalConfigStore = global.pluginConfigStore || {};
        global.pluginConfigStore = globalConfigStore;
        ...
        try {
            ...
            const existingPlugins = await appwrite.databases.listDocuments(process.env.APPWRITE_DATABASE_ID || 'bigtechdb', 'plugins', [node_appwrite_1.Query.equal('tenantId', tenantId), node_appwrite_1.Query.equal('pluginId', pluginId)]);
            const pluginData = {
                tenantId,
                pluginId,
            ...
            if (existingPlugins.documents.length > 0) {
                // Atualizar
                await appwrite.databases.updateDocument(process.env.APPWRITE_DATABASE_ID || 'bigtechdb', 'plugins', existingPlugins.documents[0].$id, pluginData);
            }
            else {
                // Criar
                await appwrite.databases.createDocument(process.env.APPWRITE_DATABASE_ID || 'bigtechdb', 'plugins', 'unique()', pluginData);
            }
        }
        catch (error) {
...
router.delete('/:pluginId', async (req, res) => {
    try {
        ...
        const plugins = await appwrite.databases.listDocuments(process.env.APPWRITE_DATABASE_ID || 'bigtechdb', 'plugins', [node_appwrite_1.Query.equal('tenantId', tenantId), node_appwrite_1.Query.equal('pluginId', pluginId)]);
...
router.post('/global/:pluginId/install', async (req, res) => {
    try {
        ...
        try {
            const existing = await appwrite.databases.listDocuments(process.env.APPWRITE_DATABASE_ID || 'bigtechdb', 'global_plugins', [node_appwrite_1.Query.equal('pluginId', pluginId)]);
            if (existing.documents.length > 0) {
        ...
        const pluginDoc = await appwrite.databases.createDocument(process.env.APPWRITE_DATABASE_ID || 'bigtechdb', 'global_plugins', 'unique()', {
            pluginId,
            type: pluginExists.type,
            version: pluginExists.version,
            status: 'installed',
            config: JSON.stringify(config || {}),
            installedBy: req.userId || 'system',
            installedAt: new Date().toISOString(),
            updatedAt: new Date().toISOString()
        });
        res.status(201).json({
...
router.put('/global/:pluginId/configure', async (req, res) => {
    try {
        ...
        const plugins = await appwrite.databases.listDocuments(process.env.APPWRITE_DATABASE_ID || 'bigtechdb', 'global_plugins', [node_appwrite_1.Query.equal('pluginId', pluginId)]);
        if (plugins.documents.length === 0) {
        ...
        const secureConfig = { ...config };
        if (secureConfig.apiKey) {
            secureConfig.apiKey = encryption_1.EncryptionUtils.encrypt(secureConfig.apiKey);
        }
        // Atualizar configuração
        const updatedDoc = await appwrite.databases.updateDocument(process.env.APPWRITE_DATABASE_ID || 'bigtechdb', 'global_plugins', pluginDoc.$id, {
            config: JSON.stringify(secureConfig),
            status: 'configured',
            updatedAt: new Date().toISOString()
        });
...
router.delete('/global/:pluginId/uninstall', async (req, res) => {
    try {
        ...
        await appwrite.databases.deleteDocument(process.env.APPWRITE_DATABASE_ID || 'bigtechdb', 'global_plugins', pluginDoc.$id);
        res.json({
            pluginId,
...
router.post('/global/:pluginId/test-connection', async (req, res) => {
    try {
        ...
        let testResult = { success: false, error: 'Test not implemented for this plugin type', details: { attempts: 0, totalTime: 0 } };
...
router.get('/global', async (req, res) => {
    try {
        const globalPlugins = await appwrite.databases.listDocuments(process.env.APPWRITE_DATABASE_ID || 'bigtechdb', 'global_plugins', [], // Sem filtros, listar todos
...
router.get('/:pluginId/services', async (req, res) => {
    try {
        ...
        if (plugin.getAvailableServices) {
            ...
            let tenantConfig = {};
            const globalConfigStore = global.pluginConfigStore || pluginConfigStore;
            const tenantConfigs = globalConfigStore[tenantId];
...
async function testConnection(url, options = {}) {
    const { method = 'GET', headers = {}, body, testOptions = {} } = options;
    const { timeout = 10000, retries = 2, retryDelay = 1000, exponentialBackoff = true } = testOptions;
    let lastError = '';
    let attempts = 0;
...
async function testConsultaPluginConnection(pluginId, config, testOptions) {
    try {
        switch (pluginId) {
            case 'bigtech':
...
async function testPagamentoPluginConnection(pluginId, config, testOptions) {
    try {
        switch (pluginId) {
            case 'asaas':
...
async function testBigTechConnection(config, testOptions) {
    try {
        ...
        const apiKey = encryption_1.EncryptionUtils.decryptIfNeeded(config.apiKey);
...
async function testInfoSimplesConnection(config, testOptions) {
    try {
...
async function testASAASConnection(config, testOptions) {
    try {
        if (!config.baseUrl || !config.apiKey) {
            return {
                success: false,

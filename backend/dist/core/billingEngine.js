...
class BillingEngine {
    ...
    userLocks = new Map();
    async runWithUserLock(userId, fn) {
        const prev = this.userLocks.get(userId) || Promise.resolve();
        let resultPromise = prev.then(() => fn());
    ...
    async debitCredits(transaction) {
        ...
        return this.runWithUserLock(uid, async () => {
            try {
                ...
                const user = await this.appwrite.databases.getDocument(process.env.APPWRITE_DATABASE_ID || 'bigtechdb', 'users', uid);
                ...
                await this.appwrite.databases.updateDocument(process.env.APPWRITE_DATABASE_ID || 'bigtechdb', 'users', uid, {
        ...
        try {
            ...
            if (original.userId) {
                await this.runWithUserLock(original.userId, async () => {
                    ...
                    await this.appwrite.databases.updateDocument(process.env.APPWRITE_DATABASE_ID || 'bigtechdb', 'users', original.userId, { credits: this.normalizeAmount((user.credits || 0) + Math.abs(Number(refundAmount))), updatedAt: new Date().toISOString() });
                });
            }
            // Audit
            await audit_1.auditLogger.log({

...
Observação: a ativação dinâmica de plugins por tenant é suportada pelo `pluginLoader` sem necessidade de rebuild do CORE — alterações podem requerer restart de container do tenant dependendo do tipo de plugin (opcional), e essas operações devem ser registradas em `Audit` e sinalizadas via notificações no admin.
...
| 1.0.3  | 20/12/2025 | Adição de detalhes sobre fonte Infosimples em plugins de consulta para alinhar com TASK-016. |

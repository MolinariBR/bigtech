...
precedence: 6
version: 1.6.0
depends_on: [1.Project.md,2.Architecture.md,3.Structure.md,4.Entities.md,5.Pages.md]
influences: [7.Tasks.md,8.DesignSystem.md]
...
**Referência**: / (página dashboard, frontend-app)  
**Referência**: Autenticação via CORE (funcionalidade)  
**Endereço Fisico**: frontend-app/src/pages/_app.tsx  
...
**Endereço Fisico**: frontend-app/src/pages/index.tsx
...
- [ ] Notificações de mudanças de plugins
...
### US-013 - Gerenciar Configurações Globais
**Como** administrador  
**Quero** configurar parâmetros globais do sistema  
**Para** ajustar comportamento e políticas  
**Referência**: Tenant.settings, Plugin.config (entidades)  
**Referência**: /settings (página, frontend-admin)  
**Referência**: Configurações globais (funcionalidade)  
**Endereço Fisico**: frontend-admin/src/pages/settings.tsx  

**Criterios de Aceite:**
- [ ] Configurações de sistema (rate limits, temas, timeouts)
- [ ] Configurações de plugins globais (valores padrão, fallbacks)
- [ ] Configurações de billing (mínimos/máximos, retenção)
- [ ] Configurações de auditoria (retenção, alertas)
- [ ] Validação e aplicação com auditId
- [ ] Isolamento lógico (não afeta outros tenants)

### US-014 - Configurar Parâmetros Globais do Sistema
**Como** administrador  
**Quero** ajustar taxas, preços, email e SMTP  
**Para** controlar políticas e comunicações do sistema  
**Referência**: SystemSettings (entidade)  
**Referência**: /settings (página, frontend-admin)  
**Referência**: Configurações globais (funcionalidade)  
**Endereço Fisico**: frontend-admin/src/pages/settings.tsx  

**Criterios de Aceite:**
- [ ] Configuração de billing (min/max compra, valor por crédito, retenção)
- [ ] Configuração de email (from, reply-to, support)
- [ ] Configuração de SMTP (host, port, credenciais)
- [ ] Configuração de rates (rate limit padrão, multiplicador fallback)
- [ ] Validação de formatos (emails, portas, valores numéricos)
- [ ] Aplicação com auditId e confirmação modal
- [ ] Feedback de sucesso/erro com detalhes

**Notas de implementação (Docs-First / Traceability):**
- Campos sensíveis (senhas SMTP) devem ser mascarados na UI e criptografados no backend.
- Mudanças devem ser auditadas com `auditId` retornado pela API.
- Validação em tempo real usando schemas de `SystemSettings`.
- Aplicação dinâmica sem restart do sistema.
...
- **Configurações Globais**: US-013
- **MVP Focado**: Prioriza varejo; expansões (atacado) em US futuras.

## 4. Histórico de Versões
- **Versão 1.6.0** (24/12/2025): Adição de US-014 para configuração de parâmetros globais (taxas, preços, email, SMTP), alinhada com nova entidade SystemSettings.
- **Versão 1.5.0** (24/12/2025): Adição de US-013 para gerenciamento de configurações globais, alinhada com nova página /settings em 5.Pages.md.
- **Versão 1.4.0** (24/12/2025): Alinhamento completo com tenancy lógico definido em 1.Project.md a 5.Pages.md. Correção de terminologia e remoção de referências a isolamento físico.
...
- **Versão 1.2** (21 de dezembro de 2025): Correção de inconsistência de nomenclatura, alterando "frontend-user" para "frontend-app" em todas as referências para alinhamento com 3.Structure.md e 5.Pages.md.
- **Versão 1.1** (20 de dezembro de 2025): Refatoração baseada em 5.Pages.md v1.1. Atualização de US-003 a US-006 para categorias e novas rotas; adição de US-004.1, US-006.1 a US-006.3 para novas páginas.

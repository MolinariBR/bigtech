...
**Notas de implementação (Docs-First / Traceability):**
- A API do CORE deve validar configurações contra schemas e retornar `auditId` para mudanças; a UI deve exibir o `auditId` após salvar.
- Configurações devem ser organizadas em abas (Sistema, Plugins, Billing, Auditoria) seguindo padrões de 8.DesignSystem.md.
- Mudanças devem ser aplicadas dinamicamente sem restart, respeitando isolamento lógico.

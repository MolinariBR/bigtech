...
### 6.8 Componentes de Configurações
O painel administrativo inclui componentes específicos para páginas de configurações globais e por tenant. Estes padrões devem ser seguidos em `/settings` e em quaisquer telas de configuração.

- **Tabs**: Componente de navegação por abas para organizar seções de configurações (ex.: Sistema, Plugins, Billing, Auditoria). Use `Tabs` do Shadcn com labels claras e ícones. Estado ativo deve ser persistido via localStorage ou URL params.
- **Form**: Formulários estruturados para configurações com `Input`, `Select`, `Textarea`, `Switch`. Deve incluir validação em tempo real (ex.: required, format), salvar rascunhos automaticamente, e mostrar mensagens de erro amigáveis. Use `Form` do React Hook Form com Zod para schema validation.
- **Settings Card**: Card dedicado para cada grupo de configurações (ex.: "Configurações Globais do Sistema"). Deve incluir título, descrição, campos de form, e botões de ação (Salvar, Resetar).
- **Validation Feedback**: Bordas vermelhas para campos inválidos, tooltips explicativos, e resumo de erros no topo do form.
- **Save Actions**: Botões "Salvar" e "Cancelar" com confirmação para mudanças críticas. Operações assíncronas devem usar loaders e toasts de sucesso/erro. Erros devem expor `auditId` para rastreabilidade.

Implementação: reutilize `Card`, `Input`, `Select`, `Button`, `Modal/Dialog` do DS. Garanta que forms sejam acessíveis, suportem undo/redo, e validem schemas de `Tenant.settings` e `Plugin.config`.

### 6.10 Componentes para Gestão de Plugins
...
### 6.11 Componentes para Monitoramento de Billing
...
### 6.12 Componentes para Auditoria de Ações
...
- **Versão 1.3** (23 de dezembro de 2025): Adição da seção 6.8 sobre componentes de configurações (Tabs, Form, Settings Card) para suporte à página /settings.

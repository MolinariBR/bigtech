...
- [ ] TASK-013. Desenvolver Gestão de Plugins (Docs-First)
  ...
  - Página de gestão de plugins (/plugins no frontend-admin) com lista de plugins instalados/ativos por tenant, filtros por tenant/tipo, toggles para enable/disable (Plugin.status), configuração via modais (Plugin.config, ex.: apiKey, fallbackSources), validação de dependências entre plugins, ações instalar novo plugin/atualizar versão/remover, notificações de deploy/restart de containers, logs de mudanças (Audit). Componentes: Table para listagem, Modal para configuração, Button para ações. Integração com entidade Plugin, isolamento multi-tenant, pluginLoader para ativação dinâmica.
...
- [ ] TASK-016. Implementar Plugin Consulta Infosimples (https://infosimples.com/consultas/)

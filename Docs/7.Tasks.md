...
- **Versão 1.33.0** (27/12/2025): Conclusão de TASK-002 (endpoints globais) e TASK-003 (persistência Appwrite) com implementação completa dos endpoints CRUD globais para plugins (/api/admin/plugins/global/*), encriptação de API keys, testes de conectividade, queries Appwrite corrigidas com Query.equal, e validação funcional via curl. Persistência global implementada com collection 'global_plugins' e serialização JSON correta.

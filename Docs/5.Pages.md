...
* **Settings (/settings)**
  Configurações globais do sistema, organizadas em abas:

  * **Sistema**: Configurações gerais (rate limits padrão, temas, timeouts).
  * **Billing**: Valores mínimos/máximos de compra de créditos, valor por crédito, retenção de transações.
  * **Email/SMTP**: Configurações de email (from, reply-to, support) e SMTP (host, port, credenciais).
  * **Rates**: Taxas e multiplicadores (rate limit padrão, custo fallback).
  * **Plugins**: Configurações globais de plugins (valores padrão, fallbacks).
  * **Auditoria**: Retenção de logs, alertas.
...
* **Versão 1.3.0** (24/12/2025): Expansão da página /settings com abas detalhadas (Sistema, Billing, Email/SMTP, Rates, Plugins, Auditoria), alinhada com nova entidade SystemSettings.

...
Representa uma **organização lógica** dentro da plataforma.
Não implica isolamento físico ou infraestrutura dedicada.
...
interface Tenant {
  ...
  createdAt: Date;
  updatedAt: Date;
  plugins: TenantPluginConfig[]; // Atualizado para estrutura rica
  settings: TenantSettings;
}

interface TenantPluginConfig {
  pluginId: string; // ID do plugin (ex: 'consulta-bigtech')
  status: 'active' | 'inactive'; // Se o plugin está ativo para este tenant
  config?: Record<string, any>; // Configurações específicas do tenant para este plugin
}
...
* **Regras**: Criado via frontend-admin; unicidade global de `Tenant.name`. Plugins de consulta são gerenciados exclusivamente via frontend-admin, com controle hierárquico (tenant + usuário individual).
* **Relacionamentos**: 1:N com User, Consulta, Billing e Audit.

---

### 3.2 User

Representa usuários finais ou administrativos associados a um tenant.
...
* **Regras**: Autenticação via CPF/CNPJ; unicidade de `identifier` por tenant. Acesso a plugins de consulta controlado via `allowedPlugins` (subset dos plugins ativos no tenant). Campo `preferences` armazena configurações como JSON string.
* **Relacionamentos**: N:1 com Tenant; 1:N com Consulta, Billing e Audit.
...
* **Relacionamentos**: N:1 com Tenant, User e Plugin; 1:1 com Billing.
...
* **Relacionamentos**: N:1 com Tenant e User; 1:1 opcional com Consulta.
...
* **Relacionamentos**: N:1 com Tenant; opcional com User.
...
* **Unicidade**:
  `User.identifier` único por tenant; `Plugin.name` único por tenant.

* **Isolamento**:
  Todas as queries são obrigatoriamente filtradas por `tenantId`.
...
* **MVP**: Tenant, User, Plugin, Consulta, Billing, Audit.
